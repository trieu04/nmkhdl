...
Pipeline ML hoàn chỉnh: từ CSV thô (15 user × 3 hoạt động × 3 modality) → tiền xử lý DSP → 2 mô hình song song (CNN 1D, Random Forest) → đánh giá closed-set + open-set.
...
```
NMKHDL/
├── rootdata - Copy/              # Dữ liệu thô (15 user, 325 CSV)
├── src/                          # Mã nguồn
│   ├── config.py                 # Hằng số, đường dẫn
│   ├── io.py                     # Đọc CSV + build manifest
│   ├── preprocess.py             # Filter + sliding window
│   ├── features.py               # 62 đặc trưng thủ công + keystroke/touch
...
# Bước 3: Trích đặc trưng keystroke/touch (cho phân tích thêm)
python -m src.features
# → data/interim/feat_keystroke.csv, feat_touch_*.csv
